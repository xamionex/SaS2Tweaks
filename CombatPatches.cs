...
public static class CombatPatches
{
    ...
    //
    // GameMonster.GetBlockStamina(Character) returns 0-1,
    // representing the fraction of poise damage absorbed while blocking.
    // We scale the result.
    ...
    //   parryFrame    = 0.15f, which is 9 frames at 60 fps (perfect-parry window).
    //   parryCooldown = 1.0f,  which is seconds before next parry is allowed.
    //
    // AccessTools.TypeByName() lets Harmony resolve the type at runtime,
    // so we don't need a compile-time reference to the internal CharUpdate class.
    ...
    //
    // CharHarvest.DoHarvest calls Common.Rand.CoinToss(fData / 100f) for every potential drop.
    // We transpile to multiply that probability by our factor, then clamp to 1.0.
    //
    ...
    static IEnumerable<CodeInstruction> DoHarvestTranspiler(IEnumerable<CodeInstruction> instructions)
    {
        foreach (var instr in instructions)
        {
            ...
            {
                // stack: [..., probability]
                yield return new CodeInstruction(OpCodes.Call, GetDropMultiplierMethod); // stack: [..., prob, mult]
                yield return new CodeInstruction(OpCodes.Mul); // stack: [..., prob*mult]
                yield return new CodeInstruction(OpCodes.Ldc_R4, 1f); // stack: [..., prob*mult, 1.0]
                yield return new CodeInstruction(OpCodes.Call, MathMinFloat); // stack: [..., min(prob*mult,1)]
            }
